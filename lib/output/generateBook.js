 ...
 */
function processOutput(book, generator, startOutput) {
    var r = Promise(startOutput)

    // watchMode的时候简化流程
    if (!book.isWatchMode()) {
        r = r
        .then(timing.measurePromiseFunc('preparePlugins', preparePlugins))
        .then(timing.measurePromiseFunc('preparePages', preparePages))
        .then(timing.measurePromiseFunc('prepareAssets', prepareAssets))
    } else {
        r = r
        .then(timing.measurePromiseFunc('preparePlugins', preparePlugins))
        .then(timing.measurePromiseFunc('preparePages', preparePages))
...
function generateBook(generator, book, options) {
    ...
    .then(function(output) {
        ...
        if (!book.get('watchFile')) {
            logger.debug.ln('cleanup folder "' + rootFolder + '"');
            return fs.ensureFolder(rootFolder)
                .thenResolve(output);
        } else {
            logger.debug.ln(`watch mode: no need cleanup folder: ${rootFolder}`)
            return output
        }
    })

    .then(processOutput.bind(null, book, generator))
